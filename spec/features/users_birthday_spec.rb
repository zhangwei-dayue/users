...
feature "测试不同格式生日日期", :type => :feature do
  ...
  context "日期格式设置为little-endian" do
    before do
      # 把日期格式设置为little-endian
      $APP["birthday_format"] = "little-endian"
    end

    scenario "注册用户生日格式为:11-01-2016, 预期结果为2016-01-11" do
      visit "/users/sign_up"  # 浏览注册页面
      expect(page).to have_content("Sign up")
      within("#new_user") do  # 填表单
        fill_in "邮箱", with: "test1@example.com"
        fill_in "生日", with: "11-01-2016"
        fill_in "密码", with: "123456"
        fill_in "再次确认", with: "123456"
      end
    ...
    scenario "注册用户生日格式为:12.2/17, 预期结果为2017-02-12" do
      visit "/users/sign_up"  # 浏览注册页面
      expect(page).to have_content("Sign up")
      within("#new_user") do  # 填表单
        fill_in "邮箱", with: "test1@example.com"
        fill_in "生日", with: "12.2/17"
        fill_in "密码", with: "123456"
        fill_in "再次确认", with: "123456"
      end
    ...
    scenario "注册用户生日格式为:13-03 89, 预期结果为1989-03-13" do
      visit "/users/sign_up"  # 浏览注册页面
      expect(page).to have_content("Sign up")
      within("#new_user") do  # 填表单
        fill_in "邮箱", with: "test1@example.com"
        fill_in "生日", with: "13-03 89"
        fill_in "密码", with: "123456"
        fill_in "再次确认", with: "123456"
      end
  ...
  context "日期格式设置为middle-endian" do
    before do
      # 把日期格式设置为middle-endian
      $APP["birthday_format"] = "middle-endian"
    end

    scenario "注册用户生日格式为:01-11-2016, 预期结果为2016-01-11" do
      visit "/users/sign_up"  # 浏览注册页面
      expect(page).to have_content("Sign up")
      within("#new_user") do  # 填表单
        fill_in "邮箱", with: "test1@example.com"
        fill_in "生日", with: "01-11-2016"
        fill_in "密码", with: "123456"
        fill_in "再次确认", with: "123456"
      end
    ...
    scenario "注册用户生日格式为:2.12/17, 预期结果为2017-02-12" do
      visit "/users/sign_up"  # 浏览注册页面
      expect(page).to have_content("Sign up")
      within("#new_user") do  # 填表单
        fill_in "邮箱", with: "test1@example.com"
        fill_in "生日", with: "2.12/17"
        fill_in "密码", with: "123456"
        fill_in "再次确认", with: "123456"
      end
    ...
    scenario "注册用户生日格式为:03-13 89, 预期结果为1989-03-13" do
      visit "/users/sign_up"  # 浏览注册页面
      expect(page).to have_content("Sign up")
      within("#new_user") do  # 填表单
        fill_in "邮箱", with: "test1@example.com"
        fill_in "生日", with: "03-13 89"
        fill_in "密码", with: "123456"
        fill_in "再次确认", with: "123456"
      end
